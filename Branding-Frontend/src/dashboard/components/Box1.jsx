...
export default function DataGridDemo() {
  ...
  const handleProcessRowUpdate = (newRow) => {
    const updatedRows = rows.map((row) =>
      row.id === newRow.id ? { ...row, status: newRow.status } : row
    );
    setRows(updatedRows);
    return newRow;
  };
  ...
  return (
    <div className="bg-white ml-5 mr-5 mb-5 py-3">
      ...
      <div className="flex justify-end px-8 py-4">
        <input
          type="text"
          className="border  focus:outline-none focus:ring-0 focus:border-gray-600"
        />
        <button className="bg-sky-800 text-white px-2 py-2 ">
          <RiSearchLine />
        </button>
      </div>
      <Box
